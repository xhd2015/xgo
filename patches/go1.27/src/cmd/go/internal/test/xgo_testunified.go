...
// go1.27: load APIs take *modload.Loader (formerly *modload.State / moduleLoaderState)
var xgoModuleLoader *modload.Loader
...
func xgoResolveNeededImports(ctx context.Context, needPkgs []string, loadedPkgs []*load.Package) map[string]*load.Package {
	...
	if len(missing) > 0 {
		loadedMissingPkgs := load.PackagesAndErrors(xgoModuleLoader, ctx, load.PackageOpts{}, missing)
