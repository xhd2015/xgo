...
func insertTrapPoints() {
	...
	forEachFunc(func(fn *ir.Func) bool {
		...
		*/
		...
		if decl != nil && decl.Generic {
			genericName = decl.GenericName()
		}
		...
		if false {
			...
			if t.HasTParam() {
				...
				genericName = xgo_func_name.FormatFuncRefName(recvTypeName, recvPtr, fnName)
			}
		}
		...
		callTrap := ir.NewCallExpr(base.AutogeneratedPos, ir.OCALL, trap, []ir.Node{
			NewStringLit(base.AutogeneratedPos, genericName),
			takeAddr(fn, recv),
